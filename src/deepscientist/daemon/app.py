...
class DaemonApp:
    ...
    def __init__(
        ...
        *,
        ...
        claude_cfg = self.runners_config.get("claude", {})
        self.claude_runner = ClaudeRunner(
            home=home,
            repo_root=self.repo_root,
            binary=str(claude_cfg.get("binary") or "claude") if isinstance(claude_cfg, dict) else "claude",
            logger=self.logger,
            prompt_builder=self.prompt_builder,
            artifact_service=self.artifact_service,
        )
        opencode_cfg = self.runners_config.get("opencode", {})
        self.opencode_runner = OpenCodeRunner(
            home=home,
            repo_root=self.repo_root,
            binary=str(opencode_cfg.get("binary") or "opencode") if isinstance(opencode_cfg, dict) else "opencode",
            logger=self.logger,
            prompt_builder=self.prompt_builder,
            artifact_service=self.artifact_service,
        )
        ...
        self.runners = {
            ...
            "opencode": self._create_runner("opencode"),
        }
    ...
    def reload_runners_config(self) -> dict[str, object]:
        ...
        opencode_config = self.runners_config.get("opencode", {})
        if isinstance(opencode_config, dict):
            self.opencode_runner.binary = str(opencode_config.get("binary") or "opencode")
        ...
        result["claude"] = {
            ...
            "permission_mode": claude_config.get("permission_mode") if isinstance(claude_config, dict) else None,
        }
        result["opencode"] = {
            "binary": self.opencode_runner.binary,
            "permission_mode": opencode_config.get("permission_mode") if isinstance(opencode_config, dict) else None,
            "default_agent": opencode_config.get("default_agent") if isinstance(opencode_config, dict) else None,
            "variant": opencode_config.get("variant") if isinstance(opencode_config, dict) else None,
        }
        return result
    ...
    def create_quest(
        ...
        *,
        ...
        configured_runner = str(
            self.config_manager.load_named_normalized("config").get("default_runner", "codex")
